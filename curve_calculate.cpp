...
std::vector<double> calculateMTF(const std::vector<double>& psf) {
    int n_orig = psf.size();
    if (n_orig == 0) {
        return {};
    }
    ...
    // Create a vector of complex numbers and pad with zeros.
    ...
    double dc_component = std::abs(a[0]);
    if (dc_component < 1e-9) { // Avoid division by zero or very small numbers
        dc_component = 1.0;
    }

    // Calculate the magnitude for each frequency component and normalize it.
    for (int i = 0; i < mtf_size; i++) {
        mtf[i] = std::abs(a[i]) / dc_component;
    }
    return mtf;
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        ERFCalculator erfCalc(detector, binSize, 40);
        ...
        // 计算MTF
        std::vector<double> mtf = calculateMTF(psf_Gaussian);
